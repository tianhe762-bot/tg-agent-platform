...
面板包含：状态、日志浏览、热重载、温和重置、强杀子进程（均带二级确认）、备份、编辑配置、重启、更新管理、纯 AI 模式、卸载。

更新管理：检查更新（仅检测版本）/ 在线更新（标准更新保留配置、完全重置清空配置；自动备份当前版本、SHA-256 校验、失败自动回滚）/ 回滚版本 / 自定义更新源 / 查看版本 / 查看更新备份列表。

纯 AI 模式：开启后所有消息（包括以 `/` 开头的文字）都作为普通文字交给 AI 纯聊天，不执行工具、不解析斜杠命令。
